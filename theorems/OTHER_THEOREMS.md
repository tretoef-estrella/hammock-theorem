...
A note on provenance, stated plainly. The Hammock was built inside a longer campaign (internal name "Chaise Longue") aimed at *all* dimensions $k$. Several of the standalones below are therefore **uniform-in-$k$ campaign theorems**, calibrated to and consumed by dimension six ($k=3$) here; the Sofa ($k=2$) is their strongest cross-calibration. Where a result is uniform in $k$, that is a strength, not a dilution: the same mechanism seals dimension four as an instance and dimension six as the theorem. Where a result is specific to dimension six (the collar that grows with $q$, the eigenvector closure), it is marked as such.

---

## The eleven key theorems

These are the load-bearing links, in the order the proof consumes them. Full table with one-line summaries is in the [README](../README.md).

| # | File | Role | Scope |
|---|------|------|-------|
| 1 | [object-formal-definition.md](supporting/object-formal-definition.md) | the arrangement, the ideal, the truncated quotient, fixed formally | dim 6 |
| 2 | [odd-symmetric-theorem.md](key/odd-symmetric-theorem.md) | $E=\bigcap_J I_J$, radical; $R$ the reduced CI coordinate ring | uniform in $k$ |
| 3 | [star-architecture-theorem.md](key/star-architecture-theorem.md) | complete flat classification (words $B_j$, $Z_j$); Pattern Lemma | uniform in $k$ |
| 4 | [floor-crosscut-theorem.md](key/floor-crosscut-theorem.md) | $A_3\ge P_3$; every floor coefficient by Möbius crosscut | dim 6 (instance re-derives Sofa) |
| 5 | [ds-import-note.md](key/ds-import-note.md) | the one imported ingredient: $P_3$ = the [DS] rank quantity | dim 6 |
| 6 | [recognition-theorem.md](key/recognition-theorem.md) | window census independent of tower level | uniform in $k$ |
| 7 | [unified-annihilator-law.md](key/unified-annihilator-law.md) | top zone as annihilator, $\bar\alpha_c=105\binom{c-9}{3}$, explicit pinch | uniform in $k$ |
| 8 | [covering-lemma.md](key/covering-lemma.md) | lifting polynomials $X_J$ exist; even alternating cycles | uniform in $k$ |
| 9 | [slack-species-derivation.md](key/slack-species-derivation.md) | slack composes stratum-by-stratum; not a fit | dim 6 |
| 10 | [central-defect-law.md](key/central-defect-law.md) | codim-2 defect as Čech $H^1$ of the nerve $K(3,3)$ | dim 6 |
| 11 | [level-2-mechanism.md](key/level-2-mechanism.md) | three swap families collapse to one; $\mathrm{mult}(\mathrm{sgn})=1$ | dim 6 |
| — | [ledger-collar-budget.md](key/ledger-collar-budget.md) | the quartic identity collapsing the tower to the collar budget | dim 6 |
...
- [third-coefficient.md](supporting/third-coefficient.md) — the codim-2 stratum coefficients as an all-dimensions theorem, with the confinement lemma making the list provably complete at each order.
...
- [slap1-hilbert.md](supporting/slap1-hilbert.md) · [slap2-regularity.md](supporting/slap2-regularity.md) — the Hilbert and regularity slaps feeding the threshold.
